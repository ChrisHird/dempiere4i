...
-- DROP VIEW rv_pp_order_receipt_issue;
 ...
 SELECT obl.pp_order_bomline_id,
    ...
    obl.pp_order_id
JOIN ppordboml obl ON mos.pp_order_bomline_id = obl.pp_order_bomline_id
JOIN ppordbomlt obl ON mos.pp_order_bomline_id = obl.pp_order_bomline_id
JOIN matrseti asi ON mos.m_attributesetinstance_id = asi.m_attributesetinstance_id
     JOIN c_uom u ON mos.c_uom_id = u.c_uom_id
     JOIN m_product p ON mos.m_product_id = p.m_product_id
JOIN mwhse w ON mos.m_warehouse_id = w.m_warehouse_id;
JOIN mwhsea w ON mos.m_warehouse_id = w.m_warehouse_id;
