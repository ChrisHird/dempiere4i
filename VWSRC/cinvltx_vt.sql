 ...
 SELECT il.ad_client_id,
    ...
    p.m_product_id,
        ...
        CASE
            ...
            ELSE NULL
        END AS uomsymbol,
    COALESCE(c.name, (COALESCE(pt.name, p.name) || COALESCE(productattribute(il.m_attributesetinstance_id), '')),
...
FROM cinvln il
     ...
     LEFT JOIN c_tax_trl t ON il.c_tax_id = t.c_tax_id AND uom.ad_language = t.ad_language
     LEFT JOIN m_product p ON il.m_product_id = p.m_product_id
JOIN cchrgt c ON il.c_charge_id = c.c_charge_id
JOIN cbpprd pp ON il.m_product_id = pp.m_product_id AND i.c_bpartner_id = pp.c_bpartner_id
JOIN mprodt pt ON il.m_product_id = pt.m_product_id AND uom.ad_language = pt.ad_language
...
JOIN cinvln il ON b.m_product_id = il.m_product_id
     JOIN m_product bp ON bp.m_product_id = il.m_product_id AND bp.isbom = 'Y' AND 
